...
import { PopConfirmEmailComponent } from './pop-confirm-email/pop-confirm-email.component';
...
export class EmailComponent implements OnInit {
   emailButtonDedabled :boolean = true;
  details = new Details(
  );
  ...
  constructor(private emailService: EmailServiceHttp,
  ...
  onSubmit() {
    this.emailService.addDetails(this.details).subscribe((res: any) => {
      ...
      const dialogRef = this.dialog.open(PopConfirmEmailComponent, {
        height: '400px',
        width: '800px',
        data: { valuer: this.details.name},
      });
      this.details.name = '';
      this.details.prenom = '';
      this.details.email = '';
